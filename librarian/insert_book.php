...
<html>
	...
	<body>
		<form class="cd-form" method="POST" action="#">
			<legend>Podaj szczegóły książki</legend>
				...
				<div class="icon">
					<input class="b-title" type="text" name="b_title" placeholder="Tytuł" required />
				</div>
				...
				<div class="icon">
					<input class="b-price" type="number" name="b_price" placeholder="Cena" required />
				</div>
				
				<div class="icon">
					<input class="b-copies" type="number" name="b_copies" placeholder="Liczba sztuk" required />
				</div>
				
				<br />
				<input class="b-isbn" type="submit" name="b_add" value="Dodaj książkę" />
		</form>
	<body>
	...
	<?php
		if(isset($_POST['b_add']))
		{
			...
			if(mysqli_num_rows($query->get_result()) != 0)
				echo error_with_field("Książka z tym numerem ISBN już istnieje.", "b_isbn");
			else
			{
				...
				if(!$query->execute())
					die(error_without_field("Książka nie została dodana."));
				echo success("Dodano książkę.");
			}
		}
	?>
</html>