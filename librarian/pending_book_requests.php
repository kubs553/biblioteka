...
<html>
	<head>
		<title>Oczekujące książki</title>
	...
	<body>
		<?php
			...
			if($rows == 0)
				echo "<h2 align='center'>Brak oczekujących książek</h2>";
			else
			{
				echo "<form class='cd-form' method='POST' action='#'>";
				echo "<legend>Oczekujące książki</legend>";
				...
				echo "<input type='submit' value='Odrzuć' name='l_reject' />&nbsp;&nbsp;&nbsp;&nbsp;";
				echo "<input type='submit' value='Potwierdź' name='l_grant'/>";
				echo "</div>";
				echo "</form>";
			}
			...
			if(isset($_POST['l_grant']))
			{
				...
				for($i=0; $i<$rows; $i++)
				{
					if(isset($_POST['cb_'.$i]))
					{
						...
						if(!$query->execute())
							die(error_without_field("Nie można dodać książki."));
						$requests++;
						...
						$subject = "Pomyślnie dodano książkę.";
						...
						$message = "Książka '".$title."' o numerze ISBN ".$isbn." została dodana do Twojego konta. Czas oddania to: ".$due_date.".";
						
						mail($to, $subject, $message, $header);
					}
				}
				if($requests > 0)
					echo success("Pomyślnie dodano ".$requests." książkę.");
				else
					echo error_without_field("Nie wybrano książki do zatwierdzenia.");
			}
