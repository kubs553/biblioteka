...
<html>
	...
	<body>
		...
		<?php
			...
			if($rows == 0)
				echo "<h2 align='center'>Nie posiadasz żadnych książek</h2>";
			else
			{
				echo "<form class='cd-form' method='POST' action='#'>";
				echo "<legend>Moje książki</legend>";
				...
				echo "</table><br />";
				echo "<input type='submit' name='b_return' value='Zwróć wybraną książkę' />";
				echo "</form>";
			}
			...
			if(isset($_POST['b_return']))
			{
				...
				for($i=0; $i<$rows; $i++)
					if(isset($_POST['cb_book'.$i]))
					{
						...
						$query->bind_param("ss", $_SESSION['username'], $_POST['cb_book'.$i]);
						if(!$query->execute())
							die(error_without_field("Nie można zwrócić książki"));
						...
						if($days > 0)
						{
							...
							$query->execute();
							echo '<script>
									document.getElementById("error").innerHTML += "Kara w wyskości '.$penalty.' została nałożona za przetrzymanie książki '.$_POST['cb_book'.$i].' przez '.$days.' dni po terminie.<br />";
				...
				{
					echo '<script>
							document.getElementById("success").innerHTML = "Pomyślnie zwrócono '.$books.' książkę";
					...
					echo error_without_field("Wybierz książkę do zwrotu.");
			}
		?>
