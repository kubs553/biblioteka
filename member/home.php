...
<html>
	...
	<body>
		<?php
			...
			if(!$result)
				die("Nie można było pobrać listy książek.");
			$rows = mysqli_num_rows($result);
			if($rows == 0)
				echo "<h2 align='center'>Brak książek.</h2>";
			else
			{
				echo "<form class='cd-form' method='POST' action='#'>";
				echo "<legend>Dostępne książki</legend>";
				...
				echo "</table>";
				echo "<br /><br /><input type='submit' name='m_request' value='Wypożycz książkę' />";
				echo "</form>";
			}
			...
			if(isset($_POST['m_request']))
			{
				if(empty($_POST['rd_book']))
					echo error_without_field("Wybierz książkę do wypożyczenia");
				else
				{
							...
							else
							{
								...
								else
								{
									...
									else
									{
										...
										if(!$query->execute())
											echo error_without_field("Nie można wypożyczyć książki");
										else
											echo success("Książka wypożyczona. Otrzymasz powiadomienie, kiedy zostanie przypisana do Twojego konta.");
									}
								}
							}
						}
